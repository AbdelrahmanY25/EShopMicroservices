...
{
	public async Task<Result<UpdateOrderResult>> Handle(UpdateOrderCommand command, CancellationToken cancellationToken)
	{
		...
		if (order is null)
			return Result.Failure<UpdateOrderResult>(OrderErrors.OrderNotFound);
		...
		return Result.Success(new UpdateOrderResult(true));
	}
